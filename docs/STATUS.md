...
| **R2 · Redesign implementation** | Redesign **approved 2026-09-11** (canvas "PantryHub Redesign", spec in `05_Design_System.md` §6 and `04_UX_Guidelines.md`). Implement in order: composition components → Lists → List detail → Shopping mode → Products (grouped + filter sheet) → Notes → Settings/Help → sheets replace dialogs → cross-cutting (animations, empty/error states, light-theme pass, accessibility). VP-3 incremental polish is superseded by this. | ⚠️ In progress |
